...
**On `withIssue` / `withIssueResource`**: these factories are intentionally generic. Tasks were the first issue-nested resource; History and Comments (`backend/activity.go`, `issue_history` / `issue_comments` tables) reuse the same factories with no changes — only new route lines and DB helpers were needed (comment mutations follow the task pattern: `AND issue_id = ?` in the WHERE clause, so the ownership-in-SQL discipline below extends one level deeper; comment edit/delete additionally take an optional author filter so a regular user is scoped to their own comment while an Admin/Sysadmin may act on any). Attachments remain a planned future child of an issue.
